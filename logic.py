...
DOWNLOAD_DIRECTORY  = _environ.get("DOWNLOAD_DIRECTORY",  "./bot/downloads")
DATA_DIRECTORY      = _environ.get("DATA_DIRECTORY",      "./bot/data")
COOKIES_DIRECTORY   = _environ.get("COOKIES_DIRECTORY",   "./bot/data/cookies")
...
def _parse_cloudplay_format(text: str):
    """Parse the multi-line CloudPlay/JSON-ish format used by some streaming apps.
    ...
    def _kv(section: str) -> dict:
        pairs: dict = {}
        for m in _re.finditer(r'"(\w+)"\s*:\s*"([^"]*)"', section):
            pairs[m.group(1).lower()] = m.group(2)
        return pairs

    headers = _kv(header_raw)
    body    = _kv(body_raw)

    raw_url = (body.get("mpd_url") or body.get("url") or
               body.get("stream_url") or body.get("hls_url") or
               body.get("m3u8_url") or "")
    if not raw_url:
        return None
    ...
    user_agent  = headers.get("user_agent", "") or headers.get("useragent", "")
    cookie      = headers.get("cookie", "")
    referer     = headers.get("referer", "")

    # Strip the key:value blocks to find -t and filename in the trailing text
    remaining = _re.sub(r'"[^"]*"\s*:\s*"[^"]*"\s*,?\s*', "", body_raw).strip()
    ...
    if t_m:
        ...
        fn_tokens = [tok for tok in tokens_after if not tok.startswith("-")]
        if fn_tokens:
...
async def do_record(client: Client, query: CallbackQuery, setup: dict):
    ...
    try:
        ...
        # Build combined extra headers (cookie + referer)
        extra_headers: dict = {}
